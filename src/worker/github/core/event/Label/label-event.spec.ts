import { Label_event } from "./label-event";
...
describe("Label_event", () => {
  ...
  it("runs the workflow pipeline for label events", async () => {
    ...
    const result = await instance.Label_event({
      event: "label",
      data: createPayload(),
    } as any);
  ...
  it("returns success when no workflow matches", async () => {
    ...
    await expect(instance.Label_event({ event: "label", data: createPayload() } as any))
  ...
  it("marks the workflow run failed when executeActions throws", async () => {
    ...
    const result = await instance.Label_event({ event: "label", data: createPayload() } as any);
  ...
  it("returns cached viewer data when sender url exists", async () => {
    const instance = new Label_event(createPrismaMock() as any);
