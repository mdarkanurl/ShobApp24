...
import { GitHubWatchWebhook } from "./dto/github-watch-webhook.dto";
...
type WatchEventDataset = {
    event: "watch";
    data: WatchPayload;
};

type WatchPayload = GitHubWatchWebhook;

export class Watch_event extends BaseEvent<WatchPayload> {
    protected readonly eventType = "watch" as const;

    constructor(prisma?: PrismaClient) {
        super(prisma);
    }

    async Watch_event(dataset: WatchEventDataset): Promise<Class_methods_type> {
        ...
        try {
            ...
            return {
                success: false,
                message: error instanceof Error ? error.message : "Failed to process GitHub watch event",
                allUpTo: false,
                requeue: true,
            };
        }
        ...
        try {
            ...
            if (action.type === "send_email") {
                const body = await this.buildEmailBody({
                    ...
                    includeViewerInfo: "do_you_want_viewer_info" in action.config,
                    getViewerData,
                });
    ...
    protected createViewerDataLoader(payload: WatchPayload): () => Promise<Actions_function_type> {
