...
import { GitHubIssueCommentWebhook } from "./dto/github-issue_comment-webhook.dto";
...
type IssueCommentEventDataset = {
    event: "issue_comment";
    data: IssueCommentPayload;
};

type IssueCommentPayload = GitHubIssueCommentWebhook;

export class Issue_comment_event extends BaseEvent<IssueCommentPayload> {
    protected readonly eventType = "issue_comment" as const;

    constructor(prisma?: PrismaClient) {
        super(prisma);
    }

    async Issue_comment_event(dataset: IssueCommentEventDataset): Promise<Class_methods_type> {
        ...
        try {
            ...
            return {
                success: false,
                message: error instanceof Error ? error.message : "Failed to process GitHub issue_comment event",
                allUpTo: false,
                requeue: true,
            };
        }
    ...
    protected createViewerDataLoader(payload: IssueCommentPayload): () => Promise<Actions_function_type> {
