import { Issue_comment_event } from "./Issue_comment-event";
...
describe("Issue_comment_event", () => {
  ...
  it("runs the workflow pipeline for issue_comment events", async () => {
    ...
    const result = await instance.Issue_comment_event({
      event: "issue_comment",
      data: createPayload(),
    } as any);
  ...
  it("sends an email to the trigger owner when the address is resolved", async () => {
    const instance = new Issue_comment_event(createPrismaMock() as any);
