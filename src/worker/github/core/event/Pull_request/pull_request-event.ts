...
import { githubPullRequestEventSchemaDto } from "./dto/github-pull_request-webhook.dto";
...
type PullRequestEventDataset = {
    event: "pull_request";
    data: PullRequestPayload;
}

type PullRequestPayload = githubPullRequestEventSchemaDto["data"];

export class Pull_request_event extends BaseEvent<PullRequestPayload> {
    protected readonly eventType = "pull_request" as const;

    constructor(prisma?: PrismaClient) {
        super(prisma);
    }

    async Pull_request_event(dataset: PullRequestEventDataset): Promise<Class_methods_type> {
        ...
        try {
            ...
            return {
                success: false,
                message: error instanceof Error ? error.message : "Failed to process GitHub pull_request event",
                allUpTo: false,
                requeue: true,
            };
        }
    ...
    protected createViewerDataLoader(payload: PullRequestPayload): () => Promise<Actions_function_type> {
