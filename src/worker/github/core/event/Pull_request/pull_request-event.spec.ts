import { Pull_request_event } from "./pull_request-event";
...
describe("Pull_request_event", () => {
  ...
  it("runs the workflow pipeline for pull_request events", async () => {
    ...
    const result = await instance.Pull_request_event({
      event: "pull_request",
  ...
  it("loads viewer data once and caches the result", async () => {
    const instance = new Pull_request_event(createPrismaMock() as any);
