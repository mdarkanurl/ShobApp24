import { Pull_request_review_event } from "./pull_request_review-event";
...
describe("Pull_request_review_event", () => {
  ...
  it("runs the workflow pipeline for pull_request_review events", async () => {
    const prisma = createPrismaMock();
    prisma.action.findMany.mockResolvedValue([{ id: "action-1", step: 1 }]);
    prisma.workflowRun.create.mockResolvedValue({ id: "run-1" });
    const instance = new Pull_request_review_event(prisma as any);
    ...
    const result = await instance.Pull_request_review_event({
      event: "pull_request_review",
      data: createPayload({ action: "submitted" }),
    } as any);
  ...
  it("returns success when no workflow matches", async () => {
    ...
    await expect(
      instance.Pull_request_review_event({
        event: "pull_request_review",
        data: createPayload({ action: "submitted" }),
      } as any)
  ...
  it("creates and completes a workflow run when actions execute successfully", async () => {
    ...
    const payload = createPayload({ action: "submitted" });
    const result = await instance.Pull_request_review_event({
      event: "pull_request_review",
      data: payload,
    } as any);
  ...
  it("loads viewer data once and caches the result", async () => {
    const instance = new Pull_request_review_event(createPrismaMock() as any);
  ...
  it("treats send_email as unsupported because the branch is not implemented", async () => {
    const instance = new Pull_request_review_event(createPrismaMock() as any);
    ...
    expect(result).toEqual({
      success: false,
      message: "Unsupported action type: send_email",
      requeue: false,
    });
  });

  it("treats send_email_to_me as unsupported because the branch is not implemented", async () => {
    const instance = new Pull_request_review_event(createPrismaMock() as any);
    ...
    expect(result).toEqual({
      success: false,
      message: "Unsupported action type: send_email_to_me",
      requeue: false,
    });
  });
