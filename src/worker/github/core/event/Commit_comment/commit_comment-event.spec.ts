import { Commit_comment_event } from "./commit_comment-event";
...
describe("Commit_comment_event", () => {
  ...
  it("runs the workflow pipeline for commit_comment events", async () => {
    ...
    const result = await instance.Commit_comment_event({
  ...
  it("returns success when no workflow matches", async () => {
    ...
    await expect(instance.Commit_comment_event({ event: "commit_comment", data: createPayload() } as any))
  ...
  it("marks the workflow run failed when executeActions throws", async () => {
    ...
    const result = await instance.Commit_comment_event({ event: "commit_comment", data: createPayload() } as any);
  ...
  it("returns cached viewer data when sender url exists", async () => {
    const instance = new Commit_comment_event(createPrismaMock() as any);
  ...
  it("sends email with viewer info when send_email requests it", async () => {
    const instance = new Commit_comment_event(createPrismaMock() as any);
    const result = await (instance as any).executeSingleAction(
      { type: "send_email", config: { email: "dev@example.com", subject: "Subject", body: "Body", do_you_want_push_info: true } },
      createPayload(),
      jest.fn().mockResolvedValue({ success: true, data: { viewer: 1 } })
    );
