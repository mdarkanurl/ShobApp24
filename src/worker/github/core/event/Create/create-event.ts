...
import { GitHubCreateWebhook } from "./dto/github-create-webhook.dto";
...
type CreateEventDataset = {
    event: "create";
    data: CreatePayload;
};

type CreatePayload = GitHubCreateWebhook;

export class Create_event extends BaseEvent<CreatePayload> {
    protected readonly eventType = "create" as const;

    constructor(prisma?: PrismaClient) {
        super(prisma);
    }

    async Create_event(dataset: CreateEventDataset): Promise<Class_methods_type> {
        ...
        try {
            ...
            return {
                success: false,
                message: error instanceof Error ? error.message : "Failed to process GitHub create event",
                allUpTo: false,
                requeue: true,
            };
        }
    ...
    protected createViewerDataLoader(payload: CreatePayload): () => Promise<Actions_function_type> {
