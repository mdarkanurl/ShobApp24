...
import { GitHubDeleteWebhook } from "./dto/github-delete-webhook.dto";
...
type DeleteEventDataset = {
    event: "delete";
    data: DeletePayload;
};

type DeletePayload = GitHubDeleteWebhook;

export class Delete_event extends BaseEvent<DeletePayload> {
    protected readonly eventType = "delete" as const;

    constructor(prisma?: PrismaClient) {
        super(prisma);
    }

    async Delete_event(dataset: DeleteEventDataset): Promise<Class_methods_type> {
        ...
        try {
            ...
            return {
                success: false,
                message: error instanceof Error ? error.message : "Failed to process GitHub delete event",
                allUpTo: false,
                requeue: true,
            };
        }
    ...
    protected async executeSingleAction(
        ...
        try {
            ...
            if (action.type === "send_email_to_me") {
                const body = await this.buildEmailBody({
                    body: action.config.body || "",
                    includeViewerInfo: "do_you_want_push_info" in action.config ? action.config.do_you_want_push_info : false,
                    getViewerData,
                });
    ...
    protected createViewerDataLoader(payload: DeletePayload): () => Promise<Actions_function_type> {
