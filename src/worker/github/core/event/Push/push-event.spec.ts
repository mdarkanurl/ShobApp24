...
jest.mock("../../actions/ai_analytics_for_push_event", () => ({
  AI_analytics_for_push_event: jest.fn(),
}));

const { sendEmail } = require("../../../../../utils/rabbitmq");
const {
  AI_analytics_for_push_event,
} = require("../../actions/ai_analytics_for_push_event");
...
describe("Push_event", () => {
  ...
  it("sends a plain push email without push info or analytics", async () => {
    ...
    expect(AI_analytics_for_push_event).not.toHaveBeenCalled();
    expect(result).toEqual({
      success: true,
  ...
  it("sends push emails with push info and AI analytics when requested", async () => {
    const instance = new Push_event(createPrismaMock() as any);
    AI_analytics_for_push_event.mockResolvedValue({
      success: true,
