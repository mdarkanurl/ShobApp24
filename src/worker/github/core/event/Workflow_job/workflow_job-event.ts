...
import { githubWorkflowJobEventSchemaDto } from "./dto/github-workflow_job-webhook.dto";
...
type WorkflowJobEventDataset = {
    event: "workflow_job";
    data: WorkflowJobPayload;
};

type WorkflowJobPayload = githubWorkflowJobEventSchemaDto["data"];

export class Workflow_job_event extends BaseEvent<WorkflowJobPayload> {
    protected readonly eventType = "workflow_job" as const;

    constructor(prisma?: PrismaClient) {
        super(prisma);
    }

    async Workflow_job_event(dataset: WorkflowJobEventDataset): Promise<Class_methods_type> {
        ...
        try {
            ...
            return {
                success: false,
                message: error instanceof Error ? error.message : "Failed to process GitHub workflow_job event",
                allUpTo: false,
                requeue: true,
            };
        }
    ...
    protected createViewerDataLoader(payload: WorkflowJobPayload): () => Promise<Actions_function_type> {
