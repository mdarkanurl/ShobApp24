import { Star_event } from "./star-event";
...
describe("Star_event", () => {
  ...
  it("runs the workflow pipeline for star events", async () => {
    ...
    const result = await instance.Star_event({
      event: "star",
      data: createPayload(),
    } as any);
  ...
  it("returns cached viewer data when sender url exists", async () => {
    const instance = new Star_event(createPrismaMock() as any);
