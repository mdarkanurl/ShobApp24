...
import { githubWorkflowRunEventSchemaDto } from "./dto/github-workflow_run-webhook.dto";
...
type WorkflowRunEventDataset = {
    event: "workflow_run";
    data: WorkflowRunPayload;
}

type WorkflowRunPayload = githubWorkflowRunEventSchemaDto["data"];

export class Workflow_run_event extends BaseEvent<WorkflowRunPayload> {
    protected readonly eventType = "workflow_run" as const;
    
    constructor(prisma?: PrismaClient) {
        super(prisma);
    }

    async Workflow_run_event(dataset: WorkflowRunEventDataset): Promise<Class_methods_type> {
        ...
        try {
            ...
            return {
                success: false,
                message: error instanceof Error ? error.message : "Failed to process GitHub workflow_run event",
                allUpTo: false,
                requeue: true,
            };
        }
    ...
    protected createViewerDataLoader(payload: WorkflowRunPayload): () => Promise<Actions_function_type> {
